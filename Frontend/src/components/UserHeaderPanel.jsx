...
export default function UserHeaderPanel({ user = {}, role = "", notificationCount = 0, onBellClick, onLogout, onProfileClick }) {
  const initials = getInitials(user.name || user.username || "User");
  const [avatarSrc, setAvatarSrc] = useState(() => getStoredAvatar(user));

  const isAdmin = (role || user?.role || "").toString().toLowerCase() === "admin";
