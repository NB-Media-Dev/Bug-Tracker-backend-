...
export default function NotificationPopupAlerts({ role, user, onNotificationClick }) {
  ...
  const empId =
    user?.employee_id ||
    (user?.id
      ? `${role === "developer" ? "DEV" : "TS"}${String(user.id).padStart(
          3,
          "0"
        )}`
      : role === "developer"
      ? "DEV001"
      : "TS001");
  const empEmail = user?.company_email || user?.personal_email || "";

  const fetchNotifications = async () => {
    if (!empId && !empEmail) return;

    try {
      const formattedRole = role === "developer" ? "Developer" : "Tester";
      let url = `${API_BASE}/api/bugs/notifications/?role=${formattedRole}`;
      if (empId) {
        url += `&recipient_id=${empId}`;
      }
      ...
      const unreadAlerts = notifs.filter((n) => {
        ...
        return matchRole && (matchId || matchEmail || !n.recipient_id);
      });
