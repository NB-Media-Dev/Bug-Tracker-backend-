...
// import Profile from "./TesterPage/Profile";
// import Sidebar from "./TesterPage/Sidebar";
// import Helpsupport from "./TesterPage/Helpsupport";
...
// function Testerdashboard({ onLogout }) {
...
//   const test = JSON.parse(localStorage.getItem("tester_user") || "null");
...
//   useEffect(() => {
//     const activeTheme = getSavedTheme(test, "tester");
...
//       <div className="flex-1 flex flex-col min-w-0 min-h-screen transition-all duration-300 ease-in-out overflow-hidden">
...
//                                 className={`px-3 py-1 text-white font-bold text-[11px] rounded-lg shadow-sm transition-all cursor-pointer ${
//                                   isBuild && !isValidHttpUrl
...
//         <main className="p-3 sm:p-6 flex-1 overflow-x-hidden overflow-y-auto bg-[oklch(0.99_0.002_30)] dark:bg-[oklch(0.14_0.004_30)] min-w-0">
...
function Testerdashboard({ tester: propTester, onLogout }) {
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      const isCurrentlyOnReport = isReportPath(currentPath) || isReportFormDirty;
      const isTargetingReport = isReportPath(targetPath);

      if (isCurrentlyOnReport && isReportFormDirty && !isTargetingReport) {
  ...
  const navigate = (path) => {
    const isCurrentlyOnReport = isReportPath(currentPath) || isReportFormDirty;
    const isTargetingReport = isReportPath(path);
