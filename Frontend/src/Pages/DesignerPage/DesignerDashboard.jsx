...
import DesignerMyReport from "./DesignerMyReport";
import DesignerHistory from "./DesignerHistory";
import DesignerSendProject from "./DesignerSendProject";
...
function DesignerDashboard({ designer: propDesigner, developer: propDeveloper, onLogout }) {
  const [devUser, setDevUser] = useState(() => {
    const saved = localStorage.getItem("developer_user");
    return saved ? JSON.parse(saved) : (propDesigner || propDeveloper);
  });
  ...
  const developer = devUser || propDesigner || propDeveloper;
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [isCollapsed, setIsCollapsed] = useState(false);
  const [currentPath, setCurrentPath] = useState("/designer/dashboard");
  ...
  const portalTitle = "Designer Portal";
  const dashboardTitle = "Designer Dashboard";
  const supportTitle = "Designer Support & Documentation";
  ...
  return (
    <div className="flex h-screen w-screen bg-white dark:bg-slate-900 text-gray-800 dark:text-gray-100 font-sans antialiased overflow-hidden transition-colors duration-300">
      <NotificationPopupAlerts
        role="designer"
        user={developer}
        onNotificationClick={() => setCurrentPath("/designer/myreport")}
      />
      <DesignerSidebar
        open={sidebarOpen}
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full min-h-0 overflow-hidden">
        <header className="bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 h-14 sm:h-16 flex items-center justify-between px-3 sm:px-6 sticky top-0 z-30 shadow-sm shrink-0">
            ...
            <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
              <ThemeSelector currentRole="designer" user={developer} />
              ...
              <div className="relative" ref={dropdownRef}>
                <UserHeaderPanel
                  user={{ ...developer, name: devName, role: "Designer" }}
          ...
          <main className="flex-1 min-h-0 p-3 sm:p-6 overflow-x-hidden overflow-y-auto min-w-0">
            ...
            {(currentPath === "/designer/myreport" || currentPath === "/developer/myreport") && (
              <DesignerMyReport
                designer={developer}
                onNavigate={handleNavigate}
              />
